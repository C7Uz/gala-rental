<?php
/**
 * Sistema de Alquiler Mejorado - Versión Corregida
 * Soluciona el problema de verificación de WooCommerce
 */
...
function cargar_sistema_alquiler() {
    // Ahora sí, cargar todo el sistema
    ...
    /**
     * ================================================================
     * 3. PÁGINA DE LISTA DE ALQUILERES
     * ================================================================
     */
    ...
    try {
        ...
        // ========================================
        // DEBUG DETALLADO - AGREGAR ESTO
        // ========================================
        
        error_log("=== DEBUG DATOS COMPLETOS ===");
        error_log("Todos los datos POST: " . print_r($_POST, true));
        error_log("Datos productos raw: " . print_r($datos['productos'] ?? 'NO EXISTE', true));
        
        // Verificar si existe la clave productos
        if (!isset($datos['productos'])) {
            error_log("ERROR: No existe la clave 'productos' en los datos");
            return array('success' => false, 'message' => 'DEBUG: No se encontró la clave productos en el formulario');
        }
        
        if (!is_array($datos['productos'])) {
            error_log("ERROR: productos no es un array: " . gettype($datos['productos']));
            return array('success' => false, 'message' => 'DEBUG: productos no es un array');
        }
        
        error_log("Número de productos recibidos: " . count($datos['productos']));
        
        // Debug cada producto individualmente
        foreach ($datos['productos'] as $index => $producto_data) {
            error_log("=== PRODUCTO $index ===");
            error_log("Datos completos: " . print_r($producto_data, true));
            error_log("ID presente: " . (isset($producto_data['id']) ? 'SÍ (' . $producto_data['id'] . ')' : 'NO'));
            error_log("Variation ID presente: " . (isset($producto_data['variation_id']) ? 'SÍ (' . $producto_data['variation_id'] . ')' : 'NO'));
            error_log("Cantidad presente: " . (isset($producto_data['cantidad']) ? 'SÍ (' . $producto_data['cantidad'] . ')' : 'NO'));
            
            // Verificar si los valores están vacíos
            if (isset($producto_data['id'])) {
                error_log("ID vacío: " . (empty($producto_data['id']) ? 'SÍ' : 'NO'));
            }
            if (isset($producto_data['variation_id'])) {
                error_log("Variation ID vacío: " . (empty($producto_data['variation_id']) ? 'SÍ' : 'NO'));
            }
        }
        
        // ========================================
        // FIN DEBUG - REEMPLAZAR VALIDACIÓN ESTRICTA
        // ========================================
        ...
        // VALIDACIÓN MENOS ESTRICTA PARA DEBUG
        $productos_validos = array();
        $debug_info = array();
        
        foreach ($datos['productos'] as $index => $producto_data) {
            $debug_info[] = "Producto $index: ID=" . ($producto_data['id'] ?? 'FALTA') . ", Variation=" . ($producto_data['variation_id'] ?? 'FALTA');
            
            // Verificación paso a paso
            $tiene_id = !empty($producto_data['id']);
            $tiene_variation = !empty($producto_data['variation_id']);
            
            error_log("Producto $index - Tiene ID: " . ($tiene_id ? 'SÍ' : 'NO'));
            error_log("Producto $index - Tiene Variation: " . ($tiene_variation ? 'SÍ' : 'NO'));
            
            if ($tiene_id && $tiene_variation) {
                $productos_validos[] = $producto_data;
                error_log("Producto $index - AGREGADO como válido");
            } else {
                error_log("Producto $index - RECHAZADO - Falta " . (!$tiene_id ? 'ID' : '') . (!$tiene_variation ? ' Variation' : ''));
            }
        }
        
        error_log("Total productos válidos encontrados: " . count($productos_validos));
        
        if (empty($productos_validos)) {
            $mensaje_debug = 'DEBUG: No se encontraron productos válidos. Detalles: ' . implode(', ', $debug_info);
            error_log($mensaje_debug);
            
            // Crear respuesta de debug más detallada
            return array(
                'success' => false, 
                'message' => $mensaje_debug . '. Revisa la consola del navegador y verifica que seleccionaste tallas/períodos para todos los productos.'
            );
        }
...
/**
 * FUNCIÓN DE DEBUG PARA AGREGAR AL JAVASCRIPT
 * Agregar dentro de la función incluir_javascript_alquiler()
 */

function debug_javascript_adicional() {
    ?>
    <script type="text/javascript">
    // DEBUG DEL FORMULARIO
    jQuery(document).ready(function($) {
        
        // Debug al enviar formulario
        $('#form-crear-alquiler').submit(function(e) {
            ...
            $('.producto-item').each(function(index) {
                ...
                console.log(`  - Elemento variation existe: ${$item.find('input[name*="[variation_id]"]').length > 0}`);
                console.log(`  - Elemento variation checked: ${$item.find('input[name*="[variation_id]"]:checked').length > 0}`);
            });
            ...
            // No prevenir el envío por ahora para ver qué pasa
        });
        
        // Debug al seleccionar variación
        $(document).on('click', '.periodo-card, .variacion-option', function() {
            console.log('=== VARIACIÓN SELECCIONADA ===');
            const variationId = $(this).data('variation-id');
            const $input = $(this).find('input[type="radio"]');
            console.log(`Variation ID: ${variationId}`);
            console.log(`Input name: ${$input.attr('name')}`);
            console.log(`Input value: ${$input.val()}`);
            console.log(`Input checked: ${$input.prop('checked')}`);
        });
    });
    </script>
    <?php
 ...
 * Agregar al final de la función mostrar_formulario_crear_alquiler()
 */
    ...
    function incluir_javascript_alquiler() {
        ...
        jQuery(document).ready(function($) {
            ...
            // DEBUG DEL FORMULARIO - AGREGAR AL FINAL DEL JAVASCRIPT
$('#form-crear-alquiler').submit(function(e) {
    console.log('=== DEBUG ENVÍO FORMULARIO ===');
...
function mostrar_panel_debug() {
    if (isset($_GET['debug']) && current_user_can('manage_options')) {
        ?>
        <div style="position: fixed; top: 50px; right: 20px; width: 300px; background: #fff; border: 2px solid #0073aa; padding: 15px; z-index: 9999; max-height: 400px; overflow-y: auto;">
            <h4 style="margin-top: 0; color: #0073aa;">🐛 Panel de Debug</h4>
            
            <div id="debug-info">
                <button onclick="debugFormulario()" class="button">📋 Debug Formulario</button>
                <button onclick="debugProductos()" class="button">👗 Debug Productos</button>
                <button onclick="debugVariaciones()" class="button">🔧 Debug Variaciones</button>
                
                <div id="debug-output" style="margin-top: 10px; font-size: 11px; background: #f0f0f0; padding: 8px; border-radius: 3px; max-height: 200px; overflow-y: auto;"></div>
            </div>
        </div>
        
        <script>
        function debugFormulario() {
            let output = '<strong>Estado del Formulario:</strong><br>';
            ...
            if (count >= 20) output += '... (más campos)<br>';
            document.getElementById('debug-output').innerHTML = output;
        }
        
        function debugProductos() {
            let output = '<strong>Estado de Productos:</strong><br>';
            jQuery('.producto-item').each(function(index) {
                const productId = jQuery(this).find('.producto-select').val();
                const variationId = jQuery(this).find('input[name*="[variation_id]"]:checked').val();
                output += `Producto ${index + 1}:<br>`;
                output += `&nbsp;&nbsp;ID: ${productId || 'NO SELECCIONADO'}<br>`;
                output += `&nbsp;&nbsp;Variation: ${variationId || 'NO SELECCIONADO'}<br>`;
            });
            document.getElementById('debug-output').innerHTML = output;
        }
        
        function debugVariaciones() {
            let output = '<strong>Variaciones Cargadas:</strong><br>';
            jQuery('.producto-item').each(function(index) {
                const variaciones = jQuery(this).find('.periodo-card, .variacion-option').length;
                const seleccionada = jQuery(this).find('.periodo-card.selected, .variacion-option.selected').length;
                output += `Producto ${index + 1}: ${variaciones} disponibles, ${seleccionada} seleccionadas<br>`;
            });
            document.getElementById('debug-output').innerHTML = output;
        }
        </script>
...
<div style="position: fixed; bottom: 20px; right: 20px; z-index: 9998;">
    <a href="<?php echo add_query_arg('debug', '1'); ?>" class="button" style="background: #ff6b6b; color: white; text-decoration: none;">
        🐛 Activar Debug
    </a>
</div>